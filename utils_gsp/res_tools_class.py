...
class ResnetPruner:
    ...
    def get_concat_filters(self):
        # Get the total Parameters Count of the model
        sps, tot_param, non_zeros = self.get_abs_sps()
    ...
    def model_sps_at(self, threshold, cat_w, srt_l2_vals, concat_info_d):
    ...
    def get_pruned_catweights(self, threshold, cat_w, srt_l2_vals, concat_info_d):
        """
    ...
    def get_threshold_bisection(self, concat_info_d, target_sps=0.90,  epsilon = 0.0009):
        ...
        while abs( self.model_sps_at(threshold, cat_w.clone(), srt_l2_vals.clone(), concat_info_d) - target_sps ) >= epsilon and abs(high - low) > 1e-6:
            print( f" Low: {low:.4f} | high = {high:0.4f} | threshold: {threshold:.4f}")

            sps_at_thresh = self.model_sps_at(threshold, cat_w.clone(), srt_l2_vals.clone(), concat_info_d)
    ...
    def put_back_cat(self, cat_weights, dim_list):
        dim1_start = 0
        ctr = 0
        for name, param in self.model.named_parameters(): 
            if 'weight' in name and 'bn' not in name and 'downsample' not in name and 'fc' not in name:
    ...
    def prune_resnet_sps(self, target_sps):
        concat_info_d  = self.get_concat_filters()
        threshold = self.get_threshold_bisection(concat_info_d, target_sps=target_sps, epsilon =  0.00001)

        cat_w_prn = self.get_pruned_catweights(threshold, 
                                        concat_info_d['cat_weights'].clone(), 
                                        concat_info_d['srt_l2_vals'].clone(), 
                                        concat_info_d)

        self.put_back_cat(cat_w_prn, concat_info_d['dim_list'])
        print( f" The total model sparsity is: {self.get_abs_sps()}")