...
import utils_gsp.torch_projection as gsp_reg
...
class TrainerMod:
    ...
    def __init__(
        ...
        if self.gsp_device == 'cpu':
            self.gsp = gsp_vec
            print("gsp with CPU")
        elif self.gsp_device == 'gpu':
            self.gsp = gsp_gpu
            print("gsp with CUDA")
    ...
    def global_gsp(self, itr, sps=0.85):
        print(f" Grouped-SPing in current Iteration: {itr} with sparsity: {sps}")
        params_d = {}
        weight_d = {}
        shape_list = []
        pre_sps = []
        ...
        for name, param in self.model.named_parameters(): 
            ...
            if 'weight' in name:
                ...
                shape_list.append(len(weight_d[name].shape))

                if len(weight_d[name].shape) == 2:
                    # pre_sps.append(gsp_vec.sparsity(weight_d[name]))
                    param.data = self.gsp.groupedsparseproj(weight_d[name], sps).requires_grad_(True)
                    
                elif len(weight_d[name].shape) == 1:
                    # pre_sps.append(gsp_vec.sparsity(weight_d[name].view(2,-1)))
                    param.data = self.gsp.groupedsparseproj(weight_d[name].view(2,-1), sps).view(-1).requires_grad_(True)
        ...
        for epoch in train_iterator:
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    # GSP Line Added
                    if (cur_itr % 200) == 0:
                        self.global_gsp(cur_itr, self.sps)
